{ config, lib, pkgs, custom, ... }: {
  ...
  users = {
    users."${custom.user}" = {
      ...
      linger = true;
      hashedPassword =
        "$6$owCyjSRvrhb4BqHV$ZSXv9Ui0JaWb.9k5TW11AlsNabe4u4I8kFhL7javAzRX/dDgEiy9XmXLlp4y7kqDHaBGi.mRRZqXzfm/6Gc/G1";
      extraGroups = [ "wheel" ]
        ++ lib.optionals (custom.type == "normal") [ "networkmanager" ];
